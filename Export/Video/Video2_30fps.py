...
ax.set_title('Energía de Beamforming')
...
out = cv2.VideoWriter('beamforming_video2.mp4', fourcc, 30.0, size)
...
try:
    for time_idx in range(0, int(RATE / CHUNK * RECORD_SECONDS)):
        ...
        # Encontrar la máxima energía y calcular ángulos estimados
        ...
        if frame_counter % FRAMES_PER_CSV_UPDATE == 0 and csv_index < len(flight_data):
            ...
            csv_index += 1  # Avanzar al siguiente punto del CSV
        ...
        # Convertir el canvas a imagen y escribirla en el video
    ...
    # Cerrar archivos WAV y liberar el VideoWriter
    for wav_file in wav_files:
        wav_file.close()
    out.release()
