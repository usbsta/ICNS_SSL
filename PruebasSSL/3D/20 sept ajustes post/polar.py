...
zeniths = np.array(elevation_range)
r, theta = np.meshgrid(zeniths, azimuths)
cax = ax.pcolormesh(theta, r, np.zeros_like(r), cmap='viridis')
fig.colorbar(cax, ax=ax, label='Energy')
ax.set_theta_zero_location('N')
ax.set_theta_direction(-1)
ax.set_title('Beamforming Energy in Real-Time')

# Crear marcador de máxima energía
