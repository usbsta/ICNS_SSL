...
OUTPUT_FILENAME = "output_pyaudio_realtime6_time_delay.wav"
...
azimuth_range = np.arange(-90, 91, 1)  # azimuth angles in degrees
elevation_range = np.arange(-90, 91, 1)  # elevation angles in degrees
...
for time_idx in range(0, int(RATE / CHUNK * RECORD_SECONDS)):
    ...
    for az_idx, theta in enumerate(azimuth_range):
        ...
        # Calculate the delays for azimuth
        delays = []
        for pos in mic_positions_azimuth:
            x, y, z = pos
            delay = (x * np.sin(azimuth_rad)) / c
            delays.append(delay)
        delay_samples = np.round(np.array(delays) * RATE).astype(int)

        # Apply Delay and Sum Beamforming in time domain
        output_signal = np.zeros(num_samples)
        for i in range(3):  # Only first three microphones
        ...
        energy_azimuth[az_idx] = np.sum(output_signal**2)
    ...
    for el_idx, phi in enumerate(elevation_range):
        ...
        # Calculate the delays for elevation
        delays = []
        ...
        # Apply Delay and Sum Beamforming in time domain
        output_signal = np.zeros(num_samples)
        for i in range(3, 6):  # Only last three microphones
            delayed_signal = np.roll(signal_data[:, i], -delay_samples[i-3])
            output_signal += delayed_signal
        output_signal /= 3

        # Calculate the energy of the combined signal
        energy_elevation[el_idx] = np.sum(output_signal**2)
