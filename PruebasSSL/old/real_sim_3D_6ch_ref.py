    ...
    for az_idx, theta in enumerate(azimuth_range):
        ...
        for el_idx, phi in enumerate(elevation_range):
            ...
            for i, delay in enumerate(delays):
                ...
                if delay_samples > 0:
                    beamformed_signal[:num_samples - delay_samples] += signal_data[delay_samples:, i]

                else:
                    beamformed_signal[-delay_samples:] += signal_data[:num_samples + delay_samples, i]


            # Cálculo de la energía total
            energy[az_idx, el_idx] = np.sum(beamformed_signal**2)
