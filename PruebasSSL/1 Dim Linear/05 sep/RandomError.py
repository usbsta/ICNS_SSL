...
TARGET_DIFFERENCE = 80e-6
peak_threshold = 2.5e8

device_index_1 = 4  # Zoom 3
device_index_2 = 5  # Zoom 2
device_index_3 = 2  # Zoom

# with bluetooth from computer
...
plt.ion()
fig, ax = plt.subplots()
line, = ax.plot(angles_range, np.zeros(len(angles_range)),label='normal')
line2, = ax.plot(angles_range, np.zeros(len(angles_range)),label='random error')
...
for _ in range(int(RATE / CHUNK * RECORD_SECONDS)):
    ...
    for idx, angle in enumerate(angles_range):
        ...
        for i in range(CHANNELS * 3):
            ...
            random_delay = np.random.uniform(0, 200e-6) * RATE  # variación entre 0 y 1 ms en muestras
            delayed_signal2 = np.roll(combined_signal[:, i], -delay_samples[i] + int(random_delay))

            delayed_signal3 = np.roll(combined_signal2[:, i], -delay_samples[i])
            output_signal += delayed_signal
            output_signal2 += delayed_signal2 # with random error
