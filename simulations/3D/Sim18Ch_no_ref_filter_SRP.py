...
# Configura los parámetros para SRP
NFFT = 2048  # tamaño de la FFT
num_sources = 1  # número de fuentes a detectar

# Inicializa el objeto SRP
srp = pra.doa.SRP(
    L=mic_positions.T,  # Transponemos para que cada columna sea un micrófono
    fs=RATE,
    nfft=NFFT,
    c=c,
    num_src=num_sources,
    azimuth=azimuth_range * np.pi / 180,  # Conversión a radianes
    colatitude=np.pi / 2 * np.ones_like(azimuth_range),  # Buscando en el plano x-y
)
...
try:
    for time_idx in range(0, int(RATE / CHUNK * RECORD_SECONDS)):
        ...
        # Procesar con SRP
        X = np.fft.rfft(filtered_signal, n=NFFT, axis=0)  # Aplicar FFT
        srp._process(X)

        # Obtener la dirección estimada
        estimated_direction = srp.grid.azimuth[srp.grid.argmax()]
        ...
        print(f"Tiempo: {current_time:.2f} s - Estimación SRP: Azimut = {estimated_direction:.2f} rad")
...
# Rangos de azimut y elevación
azimuth_range = np.arange(-180, 181, 5) * np.pi / 180  # Radianes
elevation_range = np.arange(10, 91, 5) * np.pi / 180  # Radianes

# Posiciones de los micrófonos
mic_positions = np.array([[0, 0, 1.12], [0.1, 0, 0.92], [0.2, 0, 0.77], [0, 0.1, 0.6], [0, 0.2, 0.42], [0, 0.3, 0.02]])
...
srp = pra.srp.SRP(
    ...
    colatitude=np.pi / 2 * np.ones_like(azimuth_range)
)

# Gráfico en tiempo real
plt.ion()
fig, ax = plt.subplots(figsize=(12, 5))
cax = ax.imshow(np.zeros((len(elevation_range), len(azimuth_range))), extent=[-180, 180, 0, 90], origin='lower', aspect='auto', cmap='viridis')
fig.colorbar(cax, ax=ax, label='SRP Energy')
ax.set_xlabel('Azimuth (degrees)')
ax.set_ylabel('Elevation (degrees)')
ax.set_title('Real-time SRP Energy')
...
skip_seconds = 115
for wav_file in wav_files:
    frames_to_skip = int(skip_seconds * RATE)
    wav_file.setpos(frames_to_skip)
...
try:
    for time_idx in range(0, int(RATE / CHUNK * RECORD_SECONDS)):
        buffers = []
        for wav_file in wav_files:
            block = wav_file.readframes(CHUNK)
            if len(block) == 0:
                break
            signal_data = np.frombuffer(block, dtype=np.int32).reshape(-1, CHANNELS)
            buffers.append(signal_data)
        ...
        # Aplicar SRP
        X = np.fft.rfft(filtered_signal, n=NFFT, axis=0)
        srp._process(X)

        # Obtener energía SRP
        srp_energy = srp.grid.values
        srp_energy = srp_energy.reshape((len(azimuth_range), -1))

        # Actualizar gráfico
        cax.set_data(srp_energy.T)
        cax.set_clim(vmin=np.min(srp_energy), vmax=np.max(srp_energy))

        max_energy_idx = np.unravel_index(np.argmax(srp_energy), srp_energy.shape)
        estimated_azimuth = azimuth_range[max_energy_idx[0]] * 180 / np.pi
        estimated_elevation = elevation_range[max_energy_idx[1]] * 180 / np.pi
