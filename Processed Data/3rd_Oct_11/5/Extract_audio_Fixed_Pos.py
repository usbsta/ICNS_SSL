...
try:
    for time_idx, i in zip(range(0, int(RATE / CHUNK * RECORD_SECONDS)), range(len(flight_data))):
        ...
        az_idx = np.where(azimuth_range == 6)[0][0]
        el_idx = np.where(elevation_range == 90)[0][0]
        delays_at_0 = delay_samples[:, az_idx, el_idx]
        beamformed_signal_chunk = np.zeros(combined_signal.shape[0])
        for mic_idx in range(num_mics):
            delay = delays_at_0[mic_idx]
    ...
    # Write to WAV file
    write('beamformed_signal_az0_el0.wav', RATE, beamformed_signal_int16)
